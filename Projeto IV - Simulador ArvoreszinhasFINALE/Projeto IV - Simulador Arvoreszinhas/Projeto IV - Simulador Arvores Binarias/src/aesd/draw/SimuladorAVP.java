 ...
 * @author Bastards
 */
public class SimuladorAVP {
    
    private EngineFrame engine;
    private ArvoreVP<Integer, String> arvore;
    private Map<ArvoreVP.Node<Integer, String>, NodePosition> nodePositions;
    ...
    private static final Color RED_NODE_COLOR = new Color(220, 53, 69); 
    private static final Color BLACK_NODE_COLOR = new Color(50, 50, 50); 
    private static final Color RED_NODE_BORDER_COLOR = new Color(132, 22, 32); 
    private static final Color BLACK_NODE_BORDER_COLOR = new Color(0, 0, 0); 
    ...
    public SimuladorAVP(EngineFrame engine) {
        this.engine = engine;
        this.arvore = new ArvoreVP<>();
    ...
    private void addRandomNumber() {
        ...
        while (attempts < maxAttempts) {
            ...
            if (findNodeByKey(getRoot(), random) != null) {
                attempts++;
                continue;
            }
    ...
    private boolean wouldExceedMaxLevel(int value) {
        ArvoreVP.Node<Integer, String> parent = findInsertionParent(getRoot(), value, 0);
        if (parent == null) {
            return false; 
        }
        
        int parentLevel = getNodeLevel(parent);
        int newNodeLevel = parentLevel + 1;
        
        return newNodeLevel > MAX_LEVEL;
    }
    
    
    private ArvoreVP.Node<Integer, String> findInsertionParent(ArvoreVP.Node<Integer, String> node, int value, int currentLevel) {
        ...
        if (value < node.key) {
            if (node.left == null) {
                ...
                return findInsertionParent(node.left, value, currentLevel + 1);
            }
        } else if (value > node.key) {
            if (node.right == null) {
                return node;
            } else {
                return findInsertionParent(node.right, value, currentLevel + 1);
            }
        } else {
            return node;
        }
    ...
    private ArvoreVP.Node<Integer, String> getRoot() {
        try {
            java.lang.reflect.Field rootField = ArvoreVP.class.getDeclaredField("root");
            rootField.setAccessible(true);
            return (ArvoreVP.Node<Integer, String>) rootField.get(arvore);
        } catch (Exception e) {
            System.out.println("Erro ao acessar raiz: " + e.getMessage());
            return null;
        }
    }
    
    /**
     * Encontra um no pela chave.
     */
    private ArvoreVP.Node<Integer, String> findNodeByKey(ArvoreVP.Node<Integer, String> node, int key) {
        if (node == null) return null;
    ...
    private int getNodeLevel(ArvoreVP.Node<Integer, String> node) {
        ...
        return calculateLevelFromRoot(getRoot(), node, 0);
    }
    
    
    private int calculateLevelFromRoot(ArvoreVP.Node<Integer, String> current, 
                                      ArvoreVP.Node<Integer, String> target, int currentLevel) {
        if (current == null) return -1;
        if (current == target) return currentLevel;
        
        int leftLevel = calculateLevelFromRoot(current.left, target, currentLevel + 1);
        if (leftLevel != -1) return leftLevel;
        
        int rightLevel = calculateLevelFromRoot(current.right, target, currentLevel + 1);
        return rightLevel;
    }
    

    private boolean isNodeRed(ArvoreVP.Node<Integer, String> node) {
        if (node == null) return false;
        
        try {
            java.lang.reflect.Field colorField = node.getClass().getDeclaredField("color");
            colorField.setAccessible(true);
            Object colorValue = colorField.get(node);
            
            String colorString = colorValue.toString();
            return colorString.contains("RED") || colorString.equals("RED");
            
        } catch (Exception e) {
            System.out.println("Erro ao verificar cor do no: " + e.getMessage());
            return false;
        }
    }
    
    
    private void calculatePositions(ArvoreVP.Node<Integer, String> node, 
    ...
    private void drawTree(ArvoreVP.Node<Integer, String> node) {
        if (node == null) return;
        ...
        if (pos.level <= MAX_LEVEL) {
            boolean isRed = isNodeRed(node);
            Color nodeColor = isRed ? RED_NODE_COLOR : BLACK_NODE_COLOR;
            Color borderColor = isRed ? RED_NODE_BORDER_COLOR : BLACK_NODE_BORDER_COLOR;
            
            engine.fillCircle(pos.x, pos.y, NODE_RADIUS, nodeColor);
            engine.drawCircle(pos.x, pos.y, NODE_RADIUS, borderColor);
